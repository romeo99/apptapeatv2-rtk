...
export default function DriverStripeConnect() {
    ...
    useEffect(() => {
        const fetchDriver = async () => {
            ...
            try {
                ...
                const result = driverDoc.data();
                if (!result) {
                    console.error('Driver data is empty');
                    return;
                }

                setDriver(result as Driver);
            } catch (err) {
    ...
    const handleConnectStripe = async () => {
        try {
            ...
            const createConnectAccount = httpsCallable(functions, 'createDriverStripeConnectAccount');
            const result = await createConnectAccount({ driverId: user?.uid });
    ...
    const isStripeConnected = driver?.stripeAccountId;
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            <div className="bg-white px-4 py-4 flex items-center gap-4">
                ...
                <h1 className="text-xl font-semibold">Configurer Stripe Connect</h1>
            </div>
            ...
            <div className="p-4">
                <div className="bg-white shadow rounded-lg p-6">
                    ...
                    {isStripeConnected ? (
                        <button className="bg-blue-50 text-blue-700 p-4 rounded-md" onClick={() => getStripeDashboardUrl(driver?.stripeAccountId)}>Votre compte est déjà connecté à Stripe. Cliquez pour accédez au dashboard</button>
                    ) : (
